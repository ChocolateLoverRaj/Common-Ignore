...
const generate = async (options, baseDir = process.env.INIT_CWD || process.cwd()) => {
  ...
  const inputDir = join(baseDir, options.inputDir)

  // Join the outputDir based on baseDir.
  const outputDir = join(baseDir, options.outputDir)
  ...
  class File {
    constructor (fileName, isInput) {
      ...
      this.saved = (async () => {
        ...
        this.ready = (async () => {
          // Read the text
          const mainReadPromise = fsPromises.readFile(joinInput(fileName), 'utf8')
          ...
          if (isInput) {
            ...
            for (const dependencyFileName of files[fileName].extends) {
              ...
              if (filesMap.has(dependencyFileName)) {
                ...
                if (Object.prototype.hasOwnProperty.call(files, dependencyFileName)) {
        ...
        if (isInput) {
          ...
          const recursiveDependencyFiles = this.getDependencies()
          ...
          for (const dependencyFile of recursiveDependencyFiles) {
            // Add an empty line
            outputText += '\n'
            currentLineNumber++
          ...
          const greatestLineNumberSize = greatestLineNumber.toString().length

          // Dependency tree text
          let dependencyTreeText = '# Dependency Tree\n'

          // Loop through lines in dependency tree
          for (const dependencies of dependencyTree) {
            // The name of the file
            const fileName = dependencies[1]

            // Add the line number
            const lineNumber = zeroPad(lineMap.get(fileName), greatestLineNumberSize)
    ...
    getDependencyTree (lines = [], prefix = '') {
      // The first line
      const firstLine = prefix + this.fileName

      // Add this File
      lines.push([firstLine, this.fileName])

      // The new prefix
      const newPrefix = ' '.repeat(firstLine.length - 1) + '| --> '
  ...
  for (const fileName in files) {
    // Create a new File.
    const file = new File(fileName, true)
